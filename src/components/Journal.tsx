...
const MOOD_IDS = ["ansioso", "cansado", "grato", "paz"] as const;
const MOOD_ICONS: Record<(typeof MOOD_IDS)[number], any> = {
  ansioso: Sunset,
  cansado: Coffee,
  grato: Heart,
  paz: Smile,
...
export default function Journal({ onAskAI }: JournalProps) {
  ...
  const [selectedMood, setSelectedMood] = useState<(typeof MOOD_IDS)[number]>("ansioso");
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    onAskAI(`(${moodLabels[selectedMood]}) ${content.trim()}`);
    ...
    <div id="journal-panel" className="bg-white border border-neutral-900 rounded-lg p-6 shadow-sm font-mono text-neutral-900 transition-all flex-1 flex flex-col min-h-[320px]">
      <div className="flex items-center justify-between mb-4 shrink-0">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold tracking-wider uppercase">{t("journal.title")}</h3>
          </div>
          <p className="text-xs text-neutral-500 mt-1">{t("journal.description")}</p>
        </div>
        <button
        ...
        >
          ...
          <span>{isOpen ? t("journal.close") : t("journal.newEntry")}</span>
        </button>
      </div>
      ...
      {isOpen && (
        <form id="form-new-journal" onSubmit={handleSave} className="border border-neutral-900 p-4 rounded-lg mb-6 bg-neutral-50 animate-fadeIn shrink-0">
          <p className="text-xs font-bold uppercase tracking-wider mb-2 text-neutral-700">{t("journal.moodQuestion")}</p>
              ...
              return (
                <button
                  ...
                  className={`flex flex-col items-center p-2 rounded border text-[11px] transition-all ${
                    selectedMood === m ? "border-neutral-900 bg-neutral-900 text-white shadow-sm" : "border-neutral-200 bg-white hover:border-neutral-400 text-neutral-600"
                  }`}
                >
                  <MoodIcon className="w-4 h-4 mb-1" />
                  <span className="font-bold">{moodLabels[m]}</span>
                </button>
              );
            })}
          </div>
          ...
          <div className="flex flex-col gap-1 mb-4">
            <label className="text-[10px] text-neutral-400 uppercase font-bold">{t("journal.textareaLabel")}</label>
            <textarea
          ...
          <div className="flex justify-end gap-2">
            ...
            <button id="btn-submit-journal" type="submit" className="px-4 py-1.5 bg-neutral-900 text-white rounded text-xs hover:bg-neutral-800 transition font-bold">
              {t("journal.submit")}
            </button>
          </div>
        </form>
      ...
      {notes.length === 0 ? (
        <div className="flex-1 flex flex-col items-center justify-center text-center py-8 border border-dashed border-neutral-200 rounded-lg bg-neutral-50/50 min-h-[180px]">
          ...
          <p className="text-xs text-neutral-500">{t("journal.emptyTitle")}</p>
          <p className="text-[10px] text-neutral-400 mt-1">{t("journal.emptySubtitle")}</p>
        </div>
      ) : (
        <div className="flex-1 space-y-3 overflow-y-auto pr-1 min-h-[180px]">
          {notes.map((note) => {
            const MoodIcon = MOOD_ICONS[note.mood as (typeof MOOD_IDS)[number]] || Sunset;
            const moodLabel = moodLabels[note.mood] || note.mood;

            return (
              <div
                id={`journal-note-${note.id}`}
                key={note.id}
                onClick={() => onAskAI(note.content)}
              ...
              >
                <div className="flex items-center justify-between gap-2 mb-2">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[10px] uppercase font-bold text-neutral-800">{moodLabel}</span>
                  </div>
                ...
                <p className="text-xs text-neutral-600 leading-relaxed whitespace-pre-wrap line-clamp-3 group-hover:text-neutral-900 transition">{note.content}</p>
                ...
                <div className="mt-2.5 flex justify-between items-center text-[9px] text-neutral-400 font-bold group-hover:text-neutral-700 transition">
                  <span>{t("journal.clickForReflection")}</span>
                  <span className="opacity-0 group-hover:opacity-100 transition duration-300">{t("journal.askReflection")}</span>
                </div>
              </div>
            );
          })}
